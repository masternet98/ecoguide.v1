...
**⚠️ 모든 대화 시작 시 반드시 실행:**
1. 현재 날짜 확인 (YYYYMMDD 형식)
2. `instructions/claude.YYYYMMDD.md` 파일 존재 여부 확인
3. 파일이 없으면 반드시 생성
4. 파일이 있으면 기존 내용 확인 후 업데이트

**⚠️ 작업 완료 시 반드시 실행:**
- 해당 날짜 파일에 진행사항 업데이트
- 완료된 작업, 진행중인 작업, 다음 작업 계획 기록
...
**파일 관리 규칙**:
- 파일명: `claude.YYYYMMDD.md` (예: claude.20250916.md)
- 위치: `instructions/` 폴더
- 매일 새 파일 생성, 이전 기록 보존
- **중요**: 이는 선택사항이 아닌 필수 실행 사항입니다
