...
from src.services.batch_service import (
    get_batch_scheduler, BatchConfig, initialize_batch_system
)
...
def show_monitoring_dashboard(config: Config):
    ...
    try:
        ...
        with col1:
            st.metric(
                "총 관리 지역", 
                summary["overview"]["total_districts"],
                help="현재 모니터링 중인 시군구 수"
            )
            
        with col2:
            st.metric(
                "정상 지역", 
                summary.get("healthy_districts", 0),
                help="문제가 없는 지역 수"
            )
            
        with col3:
            error_count = len(summary["error_districts"])
            st.metric(
                "문제 지역", 
                error_count,
                delta=f"-{error_count}" if error_count > 0 else None,
                delta_color="inverse",
                help="접근 불가 또는 오류가 발생한 지역"
            )
        
        # 마지막 검사 시간
        last_check = summary["overview"]["last_check"]
        if last_check:
            last_check_dt = datetime.fromisoformat(last_check)
            time_ago = datetime.now() - last_check_dt
            if time_ago.days > 0:
            ...
            st.info(f"📅 마지막 검사: {time_str} ({last_check_dt.strftime('%Y-%m-%d %H:%M')})")
        else:
            st.warning("⚠️ 아직 모니터링 검사가 실행되지 않았습니다.")
        
        # 최근 변경사항
        if summary["recent_changes"]:
            st.subheader("📝 최근 7일간 변경사항")
            
            changes_data = []
            for change in summary["recent_changes"][:10]:  # 최근 10개만
                ...
                changes_data.append({
                    "지역": district_name,
                    "항목": url_type_name,
                    "변경유형": change["change_type"],
                    "변경시간": change_time.strftime('%m-%d %H:%M')
                })
            
            df = pd.DataFrame(changes_data)
            st.dataframe(df, use_container_width=True, hide_index=True)
        else:
            st.success("✅ 최근 7일간 변경사항이 없습니다.")
        
        # 오류 지역 표시
        if summary["error_districts"]:
            st.subheader("🚨 문제가 발생한 지역")
            
            error_data = []
            for error in summary["error_districts"]:
                ...
                url_type_name = url_type_names.get(error["url_type"], error["url_type"])
        ...
        st.error(f"대시보드 데이터 로드 중 오류가 발생했습니다: {str(e)}")

def show_manual_monitoring(config: Config):
    """
    수동 모니터링 실행 인터페이스를 표시합니다.
    """
    ...
    with st.expander("⚙️ 모니터링 설정", expanded=False):
        ...
        with col1:
            timeout = st.number_input(
                ...
                help="각 URL 요청의 최대 대기시간"
            )
            
            max_concurrent = st.number_input(
                "동시 검사 수", 
                min_value=1, max_value=10, value=5,
                help="동시에 검사할 최대 지역 수"
            )
        ...
        with col2:
            ...
            ignore_minor = st.checkbox(
                "사소한 변경사항 무시", 
                value=True,
                help="날짜/시간 등 사소한 변경사항은 무시"
            )
    ...
    if not district_keys:
        st.warning("등록된 시군구가 없습니다. 먼저 시군구 정보를 등록해주세요.")
        return
    ...
    if check_option == "특정 지역 선택":
        district_names = [key.replace('_', ' ') for key in district_keys]
        selected_names = st.multiselect(
            "검사할 지역을 선택하세요",
            district_names,
            help="여러 지역을 선택할 수 있습니다"
        )
        
        if selected_names:
            selected_districts = [
                district_keys[district_names.index(name)] 
                for name in selected_names
            ]
    
    # 실행 버튼
    if st.button("🚀 모니터링 검사 실행", type="primary"):
        if check_option == "특정 지역 선택" and not selected_districts:
            st.error("검사할 지역을 선택해주세요.")
            return
...
def run_streaming_monitoring(config: Config, monitoring_config: MonitoringConfig, 
    ...
    # 세션 상태에서 저장된 결과가 있으면 자동으로 상세 결과 표시
    if "monitoring_result" in st.session_state and st.session_state.monitoring_result:
        st.divider()
        st.subheader("📋 상세 모니터링 결과")
        
        saved_result = st.session_state.monitoring_result
        
        for district_key, district_result in saved_result["districts"].items():
            # 문제가 있는 지역은 자동 표시, 문제가 없는 지역은 수동으로 펼칠 수 있음
            has_issues = district_result["changed"] > 0 or district_result["error"] > 0
            
            # 모든 지역에 대해 expander로 표시 (문제 지역은 자동 확장)
            status_summary = "⚠️ 문제 발생" if has_issues else "✅ 정상"
            
            with st.expander(f"📍 {district_key.replace('_', ' ')} ({status_summary}, 총 {district_result['total']}개 항목)", expanded=has_issues):
                for item_result in district_result["results"]:
                    # 모든 항목 표시 (문제 항목과 정상 항목 모두)
                    status_icon = {
                        "ok": "✅",
                        "changed": "📝",
                        "error": "❌",
                        "unreachable": "🚫"
                    }.get(item_result["status"], "❓")
                    ...
                    url_type_name = url_type_names.get(item_result["url_type"], item_result["url_type"])
                    
                    col1, col2, col3 = st.columns([2, 1, 3])
                    
                    with col1:
                        st.write(f"{status_icon} **{url_type_name}**")
                    
                    with col2:
                        if item_result["status"] == "ok":
                            st.success(item_result["status"].upper())
                        elif item_result["status"] == "changed":
                            st.warning(item_result["status"].upper())
                        else:
                            st.error(item_result["status"].upper())
                    
                    with col3:
                        if item_result["error_message"]:
                            st.write(f"🔍 {item_result['error_message']}")
                        elif item_result["response_time"]:
